...
def plt_optimal_degree(
    ...
    ax[1].plot(
        cpts, err_train[0:], marker="o", label="train error", lw=2, color=dlc["dlblue"]
    )
    ax[1].plot(
        cpts, err_cv[0:], marker="o", label="cv error", lw=2, color=dlc["dlorange"]
    )
...
ltcolors_map = mpl.colors.ListedColormap(
    ltcolors
...
def plt_mc_data(
    ...
    for i in range(classes):
        ...
        ax.scatter(
            X[idx, 0], X[idx, 1], marker=m, color=map(normy(i)), s=size, label=label
        )
    if legend:
...
def plot_cat_decision_boundary(
    ax, X, predict, class_labels=None, legend=False, vector=True, color="g", lw=1
):
...
def plt_train_eq_dist(X_train, y_train, classes, X_cv, y_cv, centers, std):
    ...
    fig.canvas.footer_visible = False
    plt_mc_data(
        ax[0], X_train, y_train, classes, map=dkcolors_map, legend=True, size=50
    )
    ...
    plt_mc_data(
        ax[1], X_train, y_train, classes, map=dkcolors_map, legend=True, size=50
    )
    ax[1].set_xlabel("x0")
    ax[1].set_ylabel("x1")
    plt.show()
...
def plt_nn(model_predict, X_train, y_train, classes, X_cv, y_cv, suptitle=""):
    ...
    # add the original data to the decison boundary
    plt_mc_data(
        ax[0], X_train, y_train, classes, map=dkcolors_map, legend=True, size=75
    )
    ax[0].set_xlabel("x0")
    ax[0].set_ylabel("x1")
    ...
    plt_mc_data(
        ax[1], X_cv, y_cv, classes, map=ltcolors_map, legend=True, size=100, m="<"
    )
    ax[1].set_xlabel("x0")
    ax[1].set_ylabel("x1")
