...
def test_eval_cat_err(target):
    ...
    assert type(res_tmp) is not np.ndarray, f"The output must be an scalar but got {type(res_tmp)}"
...
def model_s_test(target, classes, input_size):
    ...
    for layer in target.layers:
        ...
        assert layer.activation == expected[i][2], \
            f"Wrong activation in layer {i}. Expected {expected[i][2]} but got {layer.activation}"
        assert layer.kernel_regularizer is None, "You must not specify any regularizer any layer"
        i = i + 1
...
def model_r_test(target, classes, input_size):
    ...
    for layer in target.layers:
        ...
        if expected[i][3] is not None:
            assert type(layer.kernel_regularizer) is expected[i][3][0], f"Wrong regularizer. Expected L2 regularizer but got {type(layer.kernel_regularizer)}"
